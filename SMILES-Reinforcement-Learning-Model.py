...
def extract_descriptors(smi):
    ...
    ri     = mol.GetRingInfo(); nr = ri.NumRings()
    sizes  = [len(r) for r in ri.AtomRings()] or [0]; avg_r = sum(sizes)/len(sizes)
    rs     = sum(1 for a in mol.GetAtoms() if a.HasProp('_CIPCode'))
    ch     = Chem.GetFormalCharge(mol)
    vec = np.array([carb,hetero,single,double,arom,wedge,nr,avg_r,rs,ch,total,0.0], dtype=float)
    vec[:10] /= max(total,1)
    return vec

# ------------------------------------------------------------------------------
# 3) Policy and multi‐head critic
# ------------------------------------------------------------------------------
class PolicyCritic(nn.Module):
    def __init__(self, in_dim, hidden=512, drop=0.2, critics=2):
        ...
        self.values = nn.ModuleList([nn.Linear(hidden,1) for _ in range(critics)])

    def forward(self, x):
        h      = self.shared(x)
        logits = torch.clamp(self.policy(h), -10, 10)
        vs     = torch.stack([vh(h).squeeze(-1) for vh in self.values], dim=1)
        return logits, vs.mean(dim=1), vs.std(dim=1)
...
def sample_perm(logits, temp=1.0):
    ...
    for _ in range(5):
        ...
        p      = torch.softmax(scores, dim=0)
        p      = torch.clamp(p, 1e-8, 1.0); p = p/p.sum()
    ...
    for _ in range(5):
        ...
        for perm, score, rem in beams:
            ...
            for i,prob in enumerate(p):
                cand.append((perm+[rem[i]],
                             score + math.log(max(prob,1e-9)),
                             rem[:i]+rem[i+1:]))
        beams = sorted(cand, key=lambda x: x[1], reverse=True)[:k]
    return "".join(str(i+1) for i in beams[0][0])

# ------------------------------------------------------------------------------
# 6) Data and Training Loop
# ------------------------------------------------------------------------------
def train():
    # Participant data
    participants = [
        {"Participant":"p1","StringSet1":"24513","SMILES1":"CCO",
         "StringSet2":"42153","SMILES2":"CC(=O)O","StringSet3":"53214","SMILES3":"c1ccccc1",
         "StringSet4":"31245","SMILES4":"C1CCCCC1","StringSet5":"35124","SMILES5":"CCN"},
        {"Participant":"p2","StringSet1":"25314","SMILES1":"CNC",
         "StringSet2":"31524","SMILES2":"CCOCC","StringSet3":"14235","SMILES3":"OCCO",
         "StringSet4":"","SMILES4":"","StringSet5":"","SMILES5":""},
        {"Participant":"p3","StringSet1":"25413","SMILES1":"CCC",
         "StringSet2":"52134","SMILES2":"CCCC","StringSet3":"21543","SMILES3":"CCCCC",
         "StringSet4":"13245","SMILES4":"CCCCCC","StringSet5":"25134","SMILES5":"CCCCCCC"},
        {"Participant":"p4","StringSet1":"25413","SMILES1":"C1=CC=CC=C1",
         "StringSet2":"43125","SMILES2":"C1CC1","StringSet3":"32514","SMILES3":"C1CCC1",
         "StringSet4":"14235","SMILES4":"C1CCCC1","StringSet5":"25134","SMILES5":"C1CCCCC1"},
        {"Participant":"p5","StringSet1":"23514","SMILES1":"CCN(CC)CC",
         "StringSet2":"12543","SMILES2":"CNCN","StringSet3":"14235","SMILES3":"CCCl",
         "StringSet4":"","SMILES4":"","StringSet5":"","SMILES5":""},
        {"Participant":"p6","StringSet1":"15234","SMILES1":"CCCCO",
         "StringSet2":"45132","SMILES2":"CCCO","StringSet3":"21435","SMILES3":"CCOCC",
         "StringSet4":"23154","SMILES4":"C1COCC1","StringSet5":"32145","SMILES5":"COC"},
        {"Participant":"p7","StringSet1":"15324","SMILES1":"C1=CC=CN=C1",
         "StringSet2":"54312","SMILES2":"C1CCNCC1","StringSet3":"12435","SMILES3":"CC(=O)NC",
         "StringSet4":"15234","SMILES4":"CCN(C)C","StringSet5":"","SMILES5":""},
        {"Participant":"p8","StringSet1":"42351","SMILES1":"CCOCCO",
         "StringSet2":"31542","SMILES2":"OCCOC","StringSet3":"","SMILES3":"",
         "StringSet4":"","SMILES4":"","StringSet5":"","SMILES5":""},
    ]
    ...
    def make_X(df_):
        ...
        for _,r in df_.iterrows():
            ...
            F.append(np.concatenate(vecs))
        return torch.tensor(np.stack(F),dtype=torch.float32)
    device  = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    X_train = make_X(train_df).to(device)
    X_val   = make_X(val_df).to(device)
    ...
    sched_rl  = OneCycleLR(opt_rl, max_lr=5e-4, total_steps=rl_epochs)
    ...
    for ep in range(1, rl_epochs+1):
        ...
        batch_r, batch_d, batch_acc, logps, sup_reg = [],[],[],[], 0.0
        ...
        for i, hum in enumerate(y_train):
            ...
            logps.append(lp)
            sup_reg += -pl_loglik(logits[i], hum, temp=0.8)
        ...
        loss        = policy_loss + value_loss - 0.01*entropy + 0.01*sup_reg

        if torch.isfinite(loss):
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt_rl.step(); sched_rl.step()

        if ep % 100 == 0:
            print(f"RL ep{ep:03d} | Loss {loss.item():.3f} | R {np.mean(batch_r):.3f} |"
                  f" D {np.mean(batch_d):.2f} | A {100*np.mean(batch_acc):.1f}%")
    ...
    train_df.to_excel("train_results.xlsx", index=False)
    ...
    val_df.to_excel("val_results.xlsx", index=False)

    print("Done. Results saved to train_results.xlsx and val_results.xlsx")
